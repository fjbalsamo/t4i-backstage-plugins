...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [azureGptPlugin],
    });

    await request(server).get('/api/azure-gpt/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/azure-gpt/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/azure-gpt/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/azure-gpt/todos/${createdTodoItem.id}`)
